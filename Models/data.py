...
class SalObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize):#(self, image_root, gt_root, depth_root, rgb_mask_root, t_mask_root, trainsize, flag):
        # self.sam = sam_model_registry["default"](checkpoint="/DATA/wangkunpeng/models/sam_vit_h_4b8939.pth")
        #
        ...
        # self.flag = flag# 2 depth 1 thermal 0 none

        #####add
        # self.rgb_masks = [rgb_mask_root + f for f in os.listdir(rgb_mask_root) if f.endswith('.png')
        #             or f.endswith('.jpg')]
        # self.t_masks = [t_mask_root + f for f in os.listdir(t_mask_root) if f.endswith('.png')
        #                   or f.endswith('.jpg')]

        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
        # self.depths = sorted(self.depths)
        # self.rgb_masks = sorted(self.rgb_masks)
        # self.t_masks = sorted(self.t_masks)
        ...
        # self.rgb_mask_transform = transforms.Compose([
        #     transforms.Resize((self.trainsize, self.trainsize)),
        #     transforms.ToTensor()])
        # self.t_mask_transform = transforms.Compose([
        #     transforms.Resize((self.trainsize, self.trainsize)),
        #     transforms.ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        #     t_mask = self.binary_loader(self.t_masks[index])
        #
        # elif flag == 1:#thermal 1
        #     depth = self.rgb_loader(self.depths[index])
        #     t_mask = self.binary_loader(self.t_masks[index])
        #
        # elif flag == 0:#rgb 0
        #     depth = self.rgb_loader(self.images[index])
        #     t_mask = self.binary_loader(self.rgb_masks[index])
        # rgb_mask = self.binary_loader(self.rgb_masks[index])
        ...
        gt = randomPeper(gt)
        # rgb_mask = randomPeper(rgb_mask)
        # t_mask = randomPeper(t_mask)
