...
class AGR(nn.Module):
    def __init__(self, in_planes):
        ...
        self.conv3x3_a = conv3x3(in_planes, in_planes)
        self.sigmoid = nn.Sigmoid()
        self.convra = conv3x3_bn_relu(in_planes, in_planes)
    def forward(self, r, a):
        a_1 = self.conv3x3_a(a)
        mask_a = self.sigmoid(a_1)
        r_a = r * mask_a
        ra = r + r_a + a
        ra = self.convra(ra)
        return ra
...
class TMSOD(nn.Module):
    def __init__(self):
        ...
        for param in self.MHSA4.parameters():
            param.requires_grad = False
        for param in self.MHSA3.parameters():
            param.requires_grad = False
        for param in self.MHSA2.parameters():
            param.requires_grad = False
        for param in self.MHSA1.parameters():
            param.requires_grad = False
        ...
        for param in self.convr4.parameters():
            param.requires_grad = False
        for param in self.convr3.parameters():
            param.requires_grad = False
        for param in self.convr2.parameters():
            param.requires_grad = False
        for param in self.convr1.parameters():
            param.requires_grad = False
        for param in self.conv_dim.parameters():
            param.requires_grad = False

        #new added parameters for tuning
        self.agr4_d = AGR(1024)
        self.agr3_d = AGR(512)
        self.agr2_d = AGR(256)
        self.agr1_d = AGR(128)
    ...
    def forward(self, rgb, a):
        ...
        r4 = self.agr4_d(fr[3], fa[3]) + fr[3]
        r3 = self.agr3_d(fr[2], fa[2]) + fr[2]
        r2 = self.agr2_d(fr[1], fa[1]) + fr[1]
        r1 = self.agr1_d(fr[0], fa[0]) + fr[0]


        flatten4 = r4.flatten(2).transpose(1, 2)# B HW dim
